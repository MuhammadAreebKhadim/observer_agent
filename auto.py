...
def run_workflow(session_log_path=None, session_id=None):
    ...
    if CAPTURE_MODE == 'screenshot':
        ...
        if os.path.exists(script_path):
            print("Running screenshot_take.py")
        ...
        if screenshot_files:
            ...
            # Run summarize_and_insert_logs.py
            script_path = os.path.join(scripts_dir, 'summarize_and_insert_logs.py')
            if os.path.exists(script_path):
                cmd = [sys.executable, script_path, latest_screenshot]
                if session_log_path:
        ...
        if os.path.exists(script_path):
            print("Running extract_multiple_frames.py")
            subprocess.run([sys.executable, script_path], check=True)
        else:
            print(f"Script not found: {script_path}")
        ...
        if screenshot_files:
            latest_screenshot = max(screenshot_files, key=os.path.getctime)
            print(f"Latest screenshot: {latest_screenshot}")
            # 4. Run summarize_and_insert_logs.py
            script_path = os.path.join(scripts_dir, 'summarize_and_insert_logs.py')
            if os.path.exists(script_path):
                cmd = [sys.executable, script_path, latest_screenshot]
                if session_log_path:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--continually', action='store_true',
                        help='Run the workflow in a loop with delay between runs')
